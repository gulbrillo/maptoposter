...
| `--no-rail` | | Skip downloading/rendering rail networks | |
...
- **Railway overlays**: Rail lines from OSM are supported as an optional layer. When present, they render using the theme's `railway` color, making transit corridors stand out without overpowering the road network. Use `--no-rail` to skip downloading and rendering rail lines.
